...
try:
    model.load_state_dict(torch.load('checkpoint_r101_tiny_bp.pth'))
    print("Loaded checkpoint_r101_tiny_bp.pth – continuing training …")
except FileNotFoundError:
    print("No existing checkpoint found – training from scratch …")

log_file = open("training_log_renn_r101_tiny_bp.txt", "w")
best_train_acc = 0.0
best_val_acc   = 0.0
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch + 1) % 5 == 0:
        torch.save(model.state_dict(), 'checkpoint_r101_tiny_bp.pth')
