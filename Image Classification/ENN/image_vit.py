...
try:
    checkpoint = torch.load('checkpoint_vit_image_bp.pth')
...
log_file = open("training_log_renn_vit_image_bp.txt", "w")
best_train_acc = 0.0
best_val_acc   = 0.0
...
for epoch in range(NUM_EPOCHS):
    ...
    for i, (images, labels) in enumerate(tqdm(trainloader, desc=f"epoch {epoch+1}/{NUM_EPOCHS}")):
        ...
        if i % 10 == 0:
            ...
            torch.save(model.state_dict(), 'checkpoint_vit_image_bp.pth')
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), 'checkpoint_vit_image_bp.pth')
        print("Checkpoint saved → checkpoint_vit_bp.pth")
