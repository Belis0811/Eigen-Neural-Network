...
import os
from EResNet_BP import ResNet152 as EResNetBP


cudnn.benchmark = True
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
cpu_cores = os.cpu_count()     
workers   = min(4 * torch.cuda.device_count(), cpu_cores) 
...
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=BATCH_SIZE, shuffle=True, num_workers=workers//2, pin_memory=True)
#trainloader = tqdm(trainloader, total=len(trainloader))

testloader = torch.utils.data.DataLoader(
    testset, batch_size=BATCH_SIZE, shuffle=False, num_workers=workers//2, pin_memory=True)

model = EResNetBP(num_classes=NUM_CLASSES)
...
try:
    checkpoint = torch.load('checkpoint_r152_image_bp.pth')
    modified_state_dict = {key.replace('module.', ''): value for key, value in checkpoint.items()}
    model.load_state_dict(modified_state_dict)
    print("Loaded checkpoint_r152_bp.pth – continuing training …")
...
log_file = open("training_log_renn_r152_image_bp.txt", "w")
best_train_acc = 0.0
best_val_acc   = 0.0
...
for epoch in range(NUM_EPOCHS):
    ...
    for i, (images, labels) in enumerate(tqdm(trainloader, desc=f"epoch {epoch+1}/{NUM_EPOCHS}")):
        ...
        if i % 10 == 0:
            ...
            torch.save(model.state_dict(), 'checkpoint_r152_image_bp.pth')
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), 'checkpoint_r152_image_bp.pth')
        print("Checkpoint saved → checkpoint_r152_bp.pth")
