...
model = EResNet().to(device)
model.assign_local_heads(num_classes=10)
# model.load_state_dict(torch.load('checkpoint_101.pth'))
...
# optim_layer2 = [optim.AdamW(block.parameters(), lr=7e-4) for block in model.layer2]
# optim_layer3 = [optim.AdamW(block.parameters(), lr=7e-4) for block in model.layer3]
...
# optim_fc = optim.AdamW(model.fc.parameters(), lr=1e-3)
optim_fc = optim.AdamW(model.fc.parameters(), lr=1e-3)
...
sched_l1    = [make_onecycle(o, 5e-4) for o in optim_layer1]
sched_l2    = [make_onecycle(o, 7e-4) for o in optim_layer2]
sched_l3    = [make_onecycle(o, 7e-4) for o in optim_layer3]
...
log_file = open("training_log_renn_r101_modify_lr.txt", "w")
best_train = 0.0
best_val = 0.0
...
for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(), 'checkpoint_101.pth')
