...
from ResNet_ENN import ResNet152 as EResNet
...
model = EResNet(num_classes=100).to(device)
model.assign_local_heads(num_classes=100)
model.load_state_dict(torch.load('checkpoint_r152_cifar100.pth'))

criterion = nn.CrossEntropyLoss()
optim_conv1 = optim.AdamW(list(model.conv1.parameters()) + list(model.bn1.parameters()),lr=1e-4)

# optim_layer1 = [optim.AdamW(block.parameters(), lr=6e-4) for block in model.layer1]
# optim_layer2 = [optim.AdamW(block.parameters(), lr=8e-4) for block in model.layer2]
# optim_layer3 = [optim.AdamW(block.parameters(), lr=8e-4) for block in model.layer3]
# optim_layer4 = [optim.AdamW(block.parameters(), lr=1e-3) for block in model.layer4]
...
sched_conv1 = make_onecycle(optim_conv1, 1e-4)
sched_fc    = make_onecycle(optim_fc,    1e-3)
sched_l1    = [make_onecycle(o, 6e-4) for o in optim_layer1]
sched_l2    = [make_onecycle(o, 8e-4) for o in optim_layer2]
sched_l3    = [make_onecycle(o, 8e-4) for o in optim_layer3]
sched_l4    = [make_onecycle(o, 1e-3) for o in optim_layer4]


#orth_loss_weight=1e-4
orth_loss_weight=1e-6


from ResNet_ENN import ENNConv2d, ENNLinear
import torch.nn.functional as F

log_file = open("training_log_renn_r152_cifar100_change_lr.txt", "w")
best_train = 0.0
best_val = 0.0
...
for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(), 'checkpoint_r152_cifar100.pth')
